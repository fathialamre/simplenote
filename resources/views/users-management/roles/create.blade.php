...
<div class="panel panel-default panel-border-color panel-border-color-primary">
    <div class="panel-heading panel-heading-divider">{{__('forms.create-role')}}<span class="panel-subtitle">{{__('forms.from-here-we-can-add-new-role')}}</span></div>
    <div class="panel-body">

        <div class="row">
            <div class="form-group col-md-6 xs-pt-10 {{ $errors->has('name') ? "has-error": ""}}">
                <label>{{ __('forms.name') }}</label>
                {{ Form::text('name','', ['class' => 'form-control input-sm', 'placeholder' => __('forms.name')]) }}
